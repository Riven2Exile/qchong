...
import game.weapon.ChongQiChuiZiWeapon;
...
public class WeaponFactory {
	...
	public static BaseWeapon getInstance(int weaponKind, int enhanceLv){
		...
		case WeaponKind.CHONG_QI_CHUI_ZI: return new ChongQiChuiZiWeapon(enhanceLv, m.min, m.max); //³äÆø´¸×Ó
		case WeaponKind.KAI_SHAN_FU: return new KaiShanFuWeapon(enhanceLv, m.min, m.max);	//¿ªÉ½¸«
		
		
		///// ÌØÊâÎäÆ÷
		case WeaponKind.RU_YI_JIN_GU_BANG: return new RuYiJinGuBangWeapon(enhanceLv, m.min, m.max);	//ÈçÒâ½ð¹¿°ô
		}
		return null;
	}
	...
	public static void getDamageValue(int weaponKind, int enhanceLv, minmax m){
		...
		else if(WeaponKind.HUAN_YING_QIANG == weaponKind){ //»ÃÓ°Ç¹
			if (0 == enhanceLv){
				m.min = 20;
				m.max = 40;
			}
		}
		else if(WeaponKind.CHONG_QI_CHUI_ZI == weaponKind){ //³äÆø´¸×Ó
			if (0 == enhanceLv){
				m.min = 20;
				m.max = 35;
			}
		}
		else if(WeaponKind.KAI_SHAN_FU == weaponKind) { //¿ªÉ½¸«
			if (0 == enhanceLv){
				m.min = 12;
				m.max = 18;
			}
